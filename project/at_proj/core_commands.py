...
def get_city_id(message):
    ...
    try:
        ...
        bot.set_state(user_id=message.from_user.id, state=CommandState.start_date, chat_id=message.chat.id)
...
def check_in_date(message):
    ...
    try:
        if re.fullmatch(pattern, message.text):
            ...
            bot.set_state(user_id=message.from_user.id, state=CommandState.end_date, chat_id=message.chat.id)
        else:
...
def check_out_date(message):
    ...
    try:
        if re.fullmatch(pattern, message.text):
            ...
            bot.set_state(user_id=message.from_user.id, state=CommandState.adults, chat_id=message.chat.id)
        else:
            msg = bot.send_message(message.chat.id, "Нужно вводить как в формате: (yyyy.mm.dd), Попробуй еще.")
            bot.register_next_step_handler(msg, check_out_date)
...
def set_price(message):
    ...
    if re.fullmatch(pattern_price, message.text):
        ...
        if max_price > min_price:
            ...
            bot.set_state(user_id=message.from_user.id, state=CommandState.destination, chat_id=message.chat.id)
...
def get_hotel_list(message):
    ...
    try:
        ...
        bot.set_state(user_id=message.from_user.id, state=CommandState.photo, chat_id=message.chat.id)
...
def set_photo_ans(message):
    ...
    if message.text.lower() == 'нет' or message.text.lower() == 'no' or message.text.lower() == 'ytn':
        ...
        bot.set_state(user_id=message.from_user.id, state=CommandState.count_photo, chat_id=message.chat.id)
