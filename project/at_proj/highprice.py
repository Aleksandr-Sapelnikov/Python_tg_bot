from loader import bot, HighState, User
import telebot
...
def high_get_city_id(message):
    ...
    try:
        ...
        bot.set_state(user_id=message.from_user.id, state=HighState.start_date, chat_id=message.chat.id)
...
@bot.message_handler(state=HighState.start_date)
def check_in_date(message):
    ...
    try:
        if re.fullmatch(pattern, message.text):
            ...
            bot.set_state(user_id=message.from_user.id, state=HighState.end_date, chat_id=message.chat.id)
        else:
...
@bot.message_handler(state=HighState.end_date)
def check_out_date(message):
    ...
    try:
        if re.fullmatch(pattern, message.text):
            ...
            bot.set_state(user_id=message.from_user.id, state=HighState.adults, chat_id=message.chat.id)
        else:
            msg = bot.send_message(message.chat.id, "Нужно вводить как в формате: (yyyy.mm.dd), Попробуй еще.")
            bot.register_next_step_handler(msg, check_out_date)
...
def set_adults(message):
    ...
    if message.text.isdigit():
        if 1 <= int(message.text) <= 6:
            ...
            bot.set_state(user_id=message.from_user.id, state=HighState.resultsSize, chat_id=message.chat.id)
        else:
...
@bot.message_handler(state=HighState.resultsSize)
    ...
    try:
        ...
        for i_hotel in range(int(user.property_dict['resultsSize'])):
            try:
                ...
                hotel_price_total = \
                    data['data']['propertySearch']['properties'][i_hotel]['price']['displayMessages'][1]['lineItems'][
                        0]['value']
        ...
        bot.set_state(user_id=message.from_user.id, state=HighState.photo, chat_id=message.chat.id)
...
@bot.message_handler(state=HighState.photo)
def set_photo_ans(message):
    ...
    if message.text.lower() == 'нет' or message.text.lower() == 'no' or message.text.lower() == 'ytn':
        ...
        bot.set_state(user_id=message.from_user.id, state=HighState.count_photo, chat_id=message.chat.id)
...
@bot.message_handler(state=HighState.count_photo)
